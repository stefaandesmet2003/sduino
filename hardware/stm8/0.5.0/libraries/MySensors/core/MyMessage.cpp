...
void MyMessage_clear(MyMessage *msg)
{
	msg->last                 = 0u;
	msg->sender               = 0u;
	msg->destination          = GATEWAY_ADDRESS; // Gateway is default destination
...
{
	if (buffer != NULL) {
		for (uint8_t i = 0; i < MyMessage_getLength(msg); i++) {
			buffer[i * 2] = convertI2H(msg->data[i] >> 4);
			buffer[(i * 2) + 1] = convertI2H(msg->data[i]);
		}
		buffer[MyMessage_getLength(msg) * 2] = '\0';
		return buffer;
	} else {
		return NULL;
	}
...
{
	if (buffer != NULL) {
		if (MyMessage_getCommand(msg) == C_STREAM) {
			return MyMessage_getCustomString(msg,buffer);
		}
		return buffer;
	} else {
		return NULL;
